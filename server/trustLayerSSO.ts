...
const APP_SLUG = 'paintpros';

export function registerTrustLayerSSO(app: Express) {

  // Trust Layer SSO Consumer — "Sign in with Trust Layer"
  app.post("/api/auth/trust-layer/login", async (req: Request, res: Response) => {
    try {
      ...
      // Return the verified user to the app — the app handles local user creation
