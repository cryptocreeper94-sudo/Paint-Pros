...
Do not include any text before or after the JSON.`
  ...
  app.get("/api/qr-code", async (req, res) => {
    try {
      ...
      const baseUrl = process.env.APP_DOMAIN || "paintpros.io";
...
IMPORTANT: NEVER use emojis in your responses - text only.`;
  ...
  app.post("/api/payments/stripe/webhook", async (req, res) => {
    try {
      ...
      // Verify webhook signature using direct Stripe SDK
      const webhookSecret = process.env.STRIPE_WEBHOOK_SECRET || process.env.PAINTPROS_WEBHOOK_SECRET;
      if (!webhookSecret) {
